...
export default function VisitorOrder() {
  ...
  const createOrder = async ({ amountPaise, notes }) => {
    let res;
    try {
      res = await fetch(`${API_BASE_URL}/razorpay-create-order.php`, {
        method: 'POST',
  ...
  const verifyPayment = async ({ razorpay_order_id, razorpay_payment_id, razorpay_signature }) => {
    const res = await fetch(`${API_BASE_URL}/razorpay-verify.php`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const startPayment = async () => {
    ...
    if (!keyId) {
      setStatus({ type: 'error', message: 'Missing VITE_RAZORPAY_KEY_ID. Payment cannot start.' });
      return;
    }
